...
export class CrudService {
    ...
    constructor(private http: Http, public confirmService: ConfirmationService) {
        ...
        this.http
            ...
            .subscribe({
                next: data => {
                    ...
                    const pageData = data.pageData || data.rows,
                        array = new Array<any>(args.pm.total);

                    pageData.forEach((element, index) => {
                        const start = (args.pm.pageNo - 1) * data.pageSize + index,
                            end = start + 1;
    ...
    private dictRequest(url: string, param: string, callback: Function, pub?: boolean) {
        // this.params.paramsMap.clear();
        // this.params.set('ids', param);
        // this.params.set('dicts', param);

        this.headers = new Headers({'Content-Type': 'application/json;charset=UTF-8'});
        this.http.post(url, JSON.stringify({'ids': param, 'dicts': param}), {headers: this.headers})
            .pipe(
                map(res => res.json()),
                catchError(err => {
                    return this.handleHttpError(err);
